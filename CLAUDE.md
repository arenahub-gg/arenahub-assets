...
5. Score/persistence via `sdk` stub: `createLocalSdk()` → `submitScore(gameId, score)`. Note: the SDK is intentionally NOT vendored — it's the one live library import, bundled into `dist/` at build time. Source builds therefore require the full repo checkout; built games stay self-contained. IMPORTANT: wrap death-path SDK calls in try/catch and always show the game-over overlay — the real platform SDK can reject (network), and the game must never soft-lock (see both reference games' `onDeath`/`onLoss`).
